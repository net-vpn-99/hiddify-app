...
- 主 API 优先：失效走备、恢复后回切；自家订阅跟着当前 API，按绑定的账号 profile 更新
- 登录 / 注册导入与手动导入分开；名称填「光速」不会覆盖账号订阅
- 全部 API 失败后 15 秒内不再反复下载配置、不再探活
- 连着时额度轮询加抖动 / 单飞 / 失败退避；间隔可读插件 `poll_quota_secs`（20–90 秒）
- 订阅刷新改读 `poll_nodes_secs`（默认 12 分钟，5–30 分钟），失败不记成功时间以便尽快重试；连接失败立刻拉一次订阅

## [1.1.15] - 2026-09-16

- 应用内客服改到 `kf.gsldone.com`
